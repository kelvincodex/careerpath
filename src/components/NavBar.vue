  ...
  <section class="fixed     w-full md:h-[100px] transition- delay-150 duration-300  ease-in-out"
    ...
    <div class="flex  w-full items-center justify-around  sr-only md:not-sr-only" :class="md">
      ...
      <div class="p-5" v-if="!auth.isLogin">
        <router-link :to="{name: ActionUtil.route.auth.login}"
          class="mr-5 text-[16px] hover:text-[#fe4066] leading-[36px] font-[400]">Login
        </router-link>
      ...
      <div class="p-5" v-else>
        ...
        <router-link :to="{name: ActionUtil.route.page.step }"
          class="rounded-md p-3.5 leading-[32px] text-[16px] font-[400] drop-shadow-2xl  transition- delay-150 duration-300  ease-in-out"
          :class="[scroll.scroll === 0 ? 'bg-gray-500 hover:bg-[#fe4066]': 'bg-[#fe4066] text-white']">Get Started
        </router-link>
      </div>
    </div>
    ...
    <div class="flex w-full justify-between items-center p-5 not-sr-only md:sr-only" v-if="show">
      ...
      <div>
        <font-awesome-icon :icon="['fas', 'bars']"
          class="hover:text-[#fe4066] hover:cursor-pointer leading-[32px] text-[29px] font-[600]"
          @click="show = false" />
      </div>
    </div>
    ...
    <div class="z-50 h-screen w-full h-screen bg-white not-sr-only md:sr-only" v-else>
      <div class="">
        ...
        <ul class=" w-full mb-5 text-center">
          <router-link :to="{name: ActionUtil.route.page.home }">
            <li class="hover:bg-[#fe4066] hover:text-white p-4">Home</li>
          </router-link>
          <router-link :to="{name: ActionUtil.route.page.about }">
            <li class="hover:bg-[#fe4066] hover:text-white p-4">About</li>
          </router-link>
          <router-link :to="{name: ActionUtil.route.page.courses }">
            <li class="hover:bg-[#fe4066] hover:text-white p-4">courses</li>
          </router-link>
        </ul>
        <ul class="w-full text-center" v-if="!auth.isLogin">
          <router-link :to="{name: ActionUtil.route.auth.login }">
            <li class="p-4 hover:text-[#fe4066]">Login</li>
          </router-link>
          <router-link :to="{name: ActionUtil.route.auth.register }">
            <li class="p-4 hover:text-[#fe4066]">Get Started</li>
          </router-link>
        </ul>
